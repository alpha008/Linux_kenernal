...
int bus_add_device(struct device *dev)
{
	...
	if (bus) {
        ...
        /*1. 创建 /sys/bus/$(bus->name)/devices/$(dev->name) 到 /sys/devices/$(dev->name) 的软连接*/
		error = sysfs_create_link(&bus->p->devices_kset->kobj,&dev->kobj, dev_name(dev));
		if (error)  goto out_groups;
        /*2.创建 /sys/devices/$(dev->name)/subsystem 到 /sys/bus/$(bus->name)/devices/$(dev->name) 的软连接*/
		error = sysfs_create_link(&dev->kobj,&dev->bus->p->subsys.kobj, "subsystem");
		if (error)  goto out_subsys;
        /*3.将 dev 加入 bus->p->klist_devices 链表*/
